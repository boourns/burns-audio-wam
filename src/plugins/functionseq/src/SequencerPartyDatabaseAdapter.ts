...
export type MultiplayerOperationDataType = RevisionType & {
  /** Timestamp */
  t: number;
};

export type MultiplayerCursorDataType = {
  /** Color of Cursor */
  color: string;
  /** Name of User */
  name: string;
...
export class SequencerPartyAdapter implements IDatabaseAdapter {
  ...
  sendOperation(operation: TextOperation): void {
    ...
    const revisionData: MultiplayerOperationDataType = {
      a: this._userId!,
      o: operation.toJSON(),
  ...
  protected _doTransaction(
    revisionId: string,
    revisionData: MultiplayerOperationDataType
  ): void {
  ...
  protected _childChanged(childSnap: firebase.database.DataSnapshot): void {
    ...
    const userData = childSnap.val() as MultiplayerCursorDataType;
